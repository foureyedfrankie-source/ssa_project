...
def create_event(request, group_id):
    ...
    if request.method == 'POST':
        ...
        total_spend_raw = request.POST.get('total_spend')

        # Convert total_spend safely
        try:
            total_spend = Decimal(total_spend_raw)
        except (TypeError, InvalidOperation):
            messages.error(request, "Invalid total spend amount.")
            return redirect('chipin:create_event', group_id=group.id)
        ...
        # Event must be at least one week from today
        if event_date < date.today() + timedelta(days=7):
            messages.error(request, "Event date must be at least one week from today.")
            return redirect('chipin:create_event', group_id=group.id)

        # Validate total spend against group max spend
        if group.group_max_spend() < total_spend:
            messages.error(request, "Total spend exceeds group max spend.")
            return redirect('chipin:create_event', group_id=group.id)
...
def transfer_funds(request, group_id, event_id):
    ...
    # Ensures that event is not Archived
    if event_status == "Archived":
        messages.error(request, "Event is archived, transfer not avaliable.")
        return redirect("chipin:group_detail", group_id=group.id)
    
    ## calculations
    share_amount = event.calculate_share()
    members = group.members.all()

    ## check to see if people have enough money
    for member in members:
        if member.profile.balance < share_amount:
            messages.error(request, f"{member.username} has insufficient funds.")
            return redirect('event_detail',group_id=group.id)

    ## Remove money from members and log this to the members (users) profile 
    total_spend = 0
    for member in members:
        member.profile.balance -= share_amount
        member.profile.save()
        ...
        total_spend += share_amount

        # Put it into admin and log again
        admin = group.admin
        admin.profile.balance += total_spend
        admin.profile.save()
    ...
    # make event archived so it can't be done again!
    event.status = "Archived"
    event.save()
    messages.success(request, "Funds transferred successfully and event archived.")
    return redirect('chipin:group_detail', group_id=group.id)
