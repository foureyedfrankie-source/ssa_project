...
def top_up_balance(request, user_id):
    if request.method == 'POST': # This is the POST request --> already had the form
        form = TopUpForm(request.POST)
        if form.is_valid(): 
            # Updates the usr balance
            amount = form.cleaned_data['amount']
            profile = request.user.profile
            profile.balance += amount
            profile.save()
            # Records transaction for security and debugging :P
    ...
    else: # This is the GET request - i need a blank one.
        form = TopUpForm()
